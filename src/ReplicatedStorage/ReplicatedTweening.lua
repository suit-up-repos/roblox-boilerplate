...
local function serverAssignProperties(instance, properties, inPlace)
...
function module:GetTweenObject(instance, tInfo, propertyTable, inPlace)
	return module:Create(instance, tInfo, propertyTable, inPlace)
end
	...
	local function Play(Yield, SpecificClient, Queue) -- this is on it's own as it needs to be called by both QueuePlay and Play
		...
		if Yield and SpecificClient == nil then
			...
			if tweenMaster.Paused == nil or tweenMaster.Paused == false then
				serverAssignProperties(instance, propertyTable, inPlace) -- assign the properties server side
			end
			return
