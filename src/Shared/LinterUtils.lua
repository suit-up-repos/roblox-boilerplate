...
function Utils.getBoxOriginWithDirection(origin, direction, range)
	...
	return CFrame.lookAt(newpos, newpos + direction * (range * 0.5))
end
...
function Utils.calculateCustomVelocity(origin, direction, range, velocity, acceleration)
	local t = range / velocity
	local yVelo = -((t ^ 2) * acceleration) / (t * 2)

	local customVelocity = Vector3.new(direction.X * velocity, yVelo, direction.Z * velocity)
